...
import { awards, accentClass } from "@/lib/data";
...
export default function Awards() {
  return (
    <ol className="space-y-8">
      {awards.map((award, i) => {
        const c = accentClass[award.color];
        return (
          <Reveal as="li" key={award.title} delay={i * 0.06}>
            <article>
              <motion.div
                whileHover={{ x: 4 }}
              ...
              >
                <div>
                  ...
                  <p className={`mt-0.5 text-xs ${c.text}`}>{award.subtitle}</p>
                </div>
